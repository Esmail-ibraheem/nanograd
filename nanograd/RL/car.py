...
ENVIRONMENT_NAME = 'MountainCar-v0'  # Update to Mars Rover environment name
...
def evaluate(model: ActorCritic, test_env: gym.Env) -> float:
    ...
    while not terminated and not truncated:
        test_env.render()  # Render the environment during evaluation
...
def run():
    ...
    for episode_number in (t := trange(EPISODES)):
        ...
        while not terminated and not truncated:
            env.render()  # Render the environment during training
